...
Fixpoint reduceCongCong (i1 m1 : nat) (a1 b1 : exp) 
 ...
 nat * (nat * (exp * (exp * list form))) :=
  match l with
  | (m2, Cong i2 a2 b2) :: l1 =>
      match Nat.lcm i1 i2 with
      | i3 =>
          match Nat.div i3 i1 with
          | n1 =>
              match Nat.div i3 i2 with
              | n2 =>
                  match
...
assert (0 < Nat.div (Nat.lcm i1 i2) i1 * m1) as Z1.
{ apply Nat.mul_pos_pos; auto. apply Nat.div_str_pos.
  split; auto. admit. }
(* cut *)
(*  (div (lcm i1 i2) i1 * m1 <= *)
...
cut (groundNExp n0 (scal (Nat.div (Nat.lcm i0 i) i0) a0)); [ intros H6 | idtac ].
cut (groundNExp n0 (scal (Nat.div (Nat.lcm i0 i) i) a)); [ intros H7 | idtac ].
cut (groundNExp n0 (scal (Nat.div (Nat.lcm i0 i) i0) b0)); [ intros H8 | idtac ].
cut (groundNExp n0 (scal (Nat.div (Nat.lcm i0 i) i) b)); [ intros H9 | idtac ].
generalize
 (reduceCongCong1_groundN (Nat.div (Nat.lcm i0 i) i0 * m1 + Nat.div (Nat.lcm i0 i) i * n)
    (Nat.lcm i0 i) n0 (Nat.div (Nat.lcm i0 i) i0 * m1) (Nat.div (Nat.lcm i0 i) i * n) _ _ _ _ H6
    H7 H8 H9);
 case
  (reduceCongCong1 (Nat.lcm i0 i) (Nat.div (Nat.lcm i0 i) i0 * m1)
     (Nat.div (Nat.lcm i0 i) i * n) (scal (Nat.div (Nat.lcm i0 i) i0) a0)
     (scal (Nat.div (Nat.lcm i0 i) i) a) (scal (Nat.div (Nat.lcm i0 i) i0) b0)
     (scal (Nat.div (Nat.lcm i0 i) i) b)
     (Nat.div (Nat.lcm i0 i) i0 * m1 + Nat.div (Nat.lcm i0 i) i * n)).
intros m3 p; case p; clear p.
intros a3 p; case p; clear p; intros b3 l3.
intros (H10, (H11, H12)).
inversion H3.
generalize (H2 (Nat.lcm i0 i) n0 m3 a3 b3 H18 H10 H11);
 case (reduceCongCong (Nat.lcm i0 i) m3 a3 b3 l0).
intros i4 p; case p; clear p.
intros m4 p; case p; clear p.
intros a4 p; case p; clear p.
